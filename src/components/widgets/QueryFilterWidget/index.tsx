...
const QueryFilterWidget: React.FC<QueryFilterWidgetProps> = ({ config, widget }) => {
  ...
  const fields = useMemo(
    () => (Array.isArray(queryFilterConfig?.queryFields) ? queryFilterConfig.queryFields : []),
  ...
  const renderField = (field: QueryFilterFieldConfig) => {
    ...
    switch (field.type) {
      ...
      case 'checkboxGroup':
        return (
          <Checkbox.Group className={`query-filter-widget__group query-filter-widget__group--${field.direction || 'horizontal'}`} options={options} />
        );
      case 'cascader':
        return (
          <Cascader
        ...
        if (field.pickerType === 'range') {
          return (
            <RangePicker
              ...
              disabledDate={field.disablePastDates ? current => !!current && current < dayjs().startOf('day') : undefined}
            />
          );
        }
        return (
      ...
      case 'radioGroup':
        return (
          <Radio.Group className={`query-filter-widget__group query-filter-widget__group--${field.direction || 'horizontal'}`} options={options} />
        );
      case 'select':
        return (
          <Select
      ...
      default:
        return (
          <Input placeholder={getFieldPlaceholder(field)} allowClear />
        );
    }
  };
  ...
  return (
    <div className="query-filter-widget">
      <Form
        form={form}
        layout="vertical"
        className="query-filter-widget__form"
      >
        <div className="query-filter-widget__fields" style={{ gap: `${fieldSpacing}px` }}>
          {fields.map(field => (
            <div
              key={field.id}
              className="query-filter-widget__field"
